...
if (strlen($_SESSION['login']) == 0) {
    ...
    if ($status == 1 && isset($_GET['reservation_id'])) {
        ...
        $pdf->MultiCell(0, 10, "Address: Morocco Agadir drarga\nPhone: +212 691115737\nEmail: agency.carrental999@gmail.com\nWebsite: https://www.carrental.com/", 0, 'R');
        ...
        // Retrieve visitor's full name from users table
        $sql = "SELECT FullName AS visitor_fullname FROM users WHERE EmailU = :useremail";
        ...
        // Debugging: Output the visitor's full name
        if (!$visitor_fullname) {
            echo "Visitor full name not found or empty.";
        } else {
            echo "Visitor full name: " . $visitor_fullname;
        }
        ...
        $sql = "SELECT r.*, c.*, b.BrandName FROM reservation r
